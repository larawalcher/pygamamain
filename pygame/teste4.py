...
def loop_principal():
    while janela.executando:
        ...
        if jogo.fim_de_jogo:
            ...
            janela.desenhar_texto("Pressione ENTER para Reiniciar ou ESC para Voltar ao Menu", 
                                  (janela.janela.get_width() // 2 - 350, 350), cor='branco', fonte=janela.fonte_pequena)
            teclas = pg.key.get_pressed()
            if teclas[pg.K_RETURN]:
                jogo.reiniciar()
            elif teclas[pg.K_ESCAPE]:
                janela.menu_inicial = True
                jogo.reiniciar()
