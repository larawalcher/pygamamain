...
def loop_principal():
    while janela.executando:
        ...
        if jogo.fim_de_jogo:
            janela.limpar('cinza')
            # Caixa com borda arredondada e sombra
            pg.draw.rect(janela.janela, CORES['cinza_escuro'], (250, 200, 680, 220), border_radius=20)
            pg.draw.rect(janela.janela, CORES['azul_claro'], (255, 205, 670, 210), border_radius=15)  # Caixa com efeito
            janela.desenhar_texto("Fim de Jogo", (janela.janela.get_width() // 2 - 150, 250), cor='vermelho', fonte=janela.fonte, sombra=True)
            janela.desenhar_texto("Pressione ENTER para Reiniciar", (janela.janela.get_width() // 2 - 250, 350), cor='branco', fonte=janela.fonte_pequena, sombra=True)
